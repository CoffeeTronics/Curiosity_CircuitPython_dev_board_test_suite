...
def run_test(pins, tx_pin=TX_PIN_NAME, rx_pin=RX_PIN_NAME, baud_rate=BAUD_RATE):
    ...
    :param list pins: list of pins to run the test on
    :param str tx_pin: pin name of UART TX
    :param str rx_pin: pin name of UART RX
    :param int baud_rate: the baudrate to use
    :return: tuple(str, list): test result followed by list of pins tested
    """
    ...
    if list(set(pins).intersection(set([tx_pin, rx_pin]))):
        ...
        # Transmit test string (must be bytes-like)
        uart.write(test_str.encode("utf-8"))
